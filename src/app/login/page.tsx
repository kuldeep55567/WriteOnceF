...
const OTPInput = ({ onComplete }: { onComplete: (otp: string) => void }) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number) => {
        if (e.key === 'Backspace' && !otp[index] && index > 0) {
            // Ensure e.target is typed as HTMLInputElement to access its properties
            const prevInput = (e.target as HTMLInputElement).parentElement?.previousSibling 
                ? ((e.target as HTMLInputElement).parentElement?.previousSibling as HTMLElement).querySelector('input') as HTMLInputElement
                : null;
            if (prevInput) {
                prevInput.focus();
                const newOtp = [...otp];
